...
/**
 ...
 @param distance The distance, in meters, that describes the maximum distance a location can be from the last known geopoint and still be accepted by the returned filter
...
/**
 Returns a filter that accepts only those locations that the device is currently in according to the Localpoint server.

 @return a filter that accepts only those locations that the device is currently in according to the Localpoint server.
 */
